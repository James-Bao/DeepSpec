...
def make_clients(server_addresses):
    """Create persistent OpenAI clients (one per server, reused across requests)."""
    return {
        addr: OpenAI(base_url=f"http://{addr}/v1", api_key="None")
        for addr in server_addresses
    }


def call_sglang(args, server_address, sample, clients, max_tokens=None):
...
def validate_server(args, server_address, probe, clients):
    start_time = time.perf_counter()
    try:
        result = call_sglang(args, server_address, dict(probe), clients, max_tokens=1)
    except Exception as exc:
...
def main():
    ...
    clients = make_clients(args.server_address)
    valid_servers = validate_servers(args, clients)
    print(f"Using servers: {valid_servers}")

    max_inflight = args.concurrency * len(valid_servers)
    ...
    with (
        ...
        ThreadPoolExecutor(max_workers=max_inflight) as executor,
    ):
        ...
        for line in input_handle:
            ...
            # Drain completed futures when at capacity (no busy-wait)
            ...
            future = executor.submit(
                call_sglang, args, server_address, sample, clients
            )
            pending.add(future)
            submitted_count += 1
            progress.update(1)

        # Drain remaining futures as they complete
